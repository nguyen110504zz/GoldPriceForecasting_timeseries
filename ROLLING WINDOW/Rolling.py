...
df_log = df['Log_Price']
print(df.head())
# %% - Chia dữ liệu thành 6 cửa sổ riêng biệt
total_length = len(df_log)
n_windows = 6
window_size = total_length // n_windows
...
for i in range(n_windows):
    ...
    window = df_log.iloc[start_idx:end_idx].copy()
    rolling_windows.append(window)

# Kiểm tra kích thước các cửa sổ
print("Kích thước mỗi cửa sổ:", [len(window) for window in rolling_windows])
print("Số lượng cửa sổ:", len(rolling_windows))
...
for i, window in enumerate(rolling_windows):
    ...
    test_size = len(window) - train_size  # 20% test

    train_set = window.iloc[:train_size].copy()
    test_set = window.iloc[train_size:].copy()
    train_test_splits.append((train_set, test_set))

# Kiểm tra kích thước train và test của mỗi cửa sổ
print("\nKích thước Train-Test trong mỗi cửa sổ:")
for i, (train, test) in enumerate(train_test_splits):
    print(f"Cửa sổ {i+1}: Train={len(train)}, Test={len(test)}")
...
for i, (train, test) in enumerate(train_test_splits):
    try:
        ...
        # Reset index cho train và test để tránh vấn đề với index
        ...
        fc_ar = fitted_ar.predict(start=len(train), end=len(train) + len(test) - 1)
        
        # Tính khoảng tin cậy
        conf_ar = fitted_ar.get_prediction(start=len(train), end=len(train) + len(test) - 1)
        conf_ar = conf_ar.conf_int(alpha=0.05)

        # Chuyển đổi từ log-price về giá gốc
        fc_ar_exp = np.exp(fc_ar)
        test_exp = np.exp(test)
        lower_series_ar_exp = np.exp(conf_ar.iloc[:, 0])
        upper_series_ar_exp = np.exp(conf_ar.iloc[:, 1])
        ...
        # Vẽ dữ liệu training
        ax.plot(range(len(train)), np.exp(train), color='blue', label="Training data")

        # Vẽ giá trị thực tế
        ax.plot(range(len(train), len(train) + len(test)), test_exp, color='orange', label="Actual Price")

        # Vẽ giá trị dự báo
        ax.plot(range(len(train), len(train) + len(test)), fc_ar_exp, color='red', label="Predicted Price")

        # Vẽ khoảng tin cậy
        ax.fill_between(range(len(train), len(train) + len(test)), 
                       lower_series_ar_exp, 
                       upper_series_ar_exp,
        ...
        print(f"Lỗi khi xử lý cửa sổ {i+1}: {str(e)}")
        print(f"Kích thước train: {len(train)}, test: {len(test)}")
        continue
...
for i, (train, test) in enumerate(train_test_splits):
    ...
    # Huấn luyện mô hình ARMA (p, d, q) = (2,0,1)
...
for i, (train, test) in enumerate(train_test_splits):
    ...
    # Vẽ giá trị dự báo
    ax.plot(fc_sarima_exp.index, fc_sarima_exp, color='red', label="Predicted Price")
...
for i, (train, test) in enumerate(train_test_splits):
    ...
    confidence_interval = 1.96 * std_dev  # Khoảng tin cậy 95%
